...
const CA_Attendance = () => {
  ...
  console.log(teacherData.courses[1].AttendanceMorning);
  return (
    <>
      <select value={section} onChange={handleSectionChange}>
        <option value={teacherData.courses[1].batchName[0]}>
          {teacherData.courses[1].batchName[0]}
        </option>
        <option value={teacherData.courses[1].batchName[1]}>
          {teacherData.courses[1].batchName[1]}
        </option>
      </select>
      <br />
      <table>
        <thead>
          <tr>
            ...
            {teacherData.courses[1].AttendanceMorning[0].attendance.map(
              (obj, index) => (
                <th
        ...
        <tbody>
          {section === "BCSF17M"
            ? teacherData.courses[1].AttendanceMorning.map((sObj, index) => {
                return (
                  <tr key={index}>
                    <td>{sObj.rollNo}</td>
            ...
            : teacherData.courses[1].AttendanceAfternoon.map((sObj, index) => {
                return (
                  <tr key={index}>
                    <td>{sObj.rollNo}</td>
                    <td>{sObj.studentName}</td>
