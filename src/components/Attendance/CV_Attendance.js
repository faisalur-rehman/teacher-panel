  ...
  return (
    <>
      <select value={section} onChange={handleSectionChange}>
        <option value={teacherData.courses[2].batchName[0]}>
          {teacherData.courses[2].batchName[0]}
        </option>
        <option value={teacherData.courses[2].batchName[1]}>
          {teacherData.courses[2].batchName[1]}
        </option>
      </select>
      <br />
      <table>
        <thead>
          <tr>
            ...
            {teacherData.courses[2].AttendanceMorning[0].attendance.map(
              (obj, index) => (
                <th
        ...
        <tbody>
          {section === "BCSF17M"
            ? teacherData.courses[2].AttendanceMorning.map((sObj, index) => {
                return (
                  <tr key={index}>
                    <td>{sObj.rollNo}</td>
            ...
            : teacherData.courses[2].AttendanceAfternoon.map((sObj, index) => {
                return (
                  <tr key={index}>
                    <td>{sObj.rollNo}</td>
                    <td>{sObj.studentName}</td>
